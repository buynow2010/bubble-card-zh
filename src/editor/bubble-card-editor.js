...
class BubbleCardEditor extends LitElement {
                        ...
                        <div class="bubble-info-container">
                            <div class="bubble-info">
                                <h4 class="bubble-section-title">
                                    ...
                                    Bubble Card ${version} 汉化版
                                </h4>
                                <div class="content">
                                    <p>如果您想了解此版本的新功能，可以查看<a href="https://github.com/Clooos/Bubble-Card/releases/tag/${version}" target="_blank" rel="noopener noreferrer"><b>更新日志</b></a>。</p>
                                </div>
                            </div>
  ...
  makeLayoutOptions() {
    ...
    return html`
        ...
        selector: {
          select: {
            options: [
              { label: '普通（旧默认）', value: 'normal' },
              { label: '大卡片', value: 'large' },
              { label: '大卡片带 2 行子按钮', value: 'large-2-rows' },
              { label: '大卡片带网格子按钮（布局：最少 2 行）', value: 'large-sub-buttons-grid' }
            ],
            mode: 'dropdown'
          }
        }
      }]}
                .computeLabel=${() => this._config.card_type === "pop-up" ? '标题卡片布局' : '卡片布局'}
                @value-changed=${(ev) => {
        this._valueChanged({
                ...
                <div class="content"> 
                    ${getLazyLoadedPanelContent(this, panelKey, !!this._expandedPanelStates[panelKey], () => html`
                        <ha-form
                            .hass=${this.hass}
                            .data=${context}
                            .configValue="${(array ? array + "." : "") + (parseInt(index) == index ? index + "." : "") + configValueType}" 
                            .schema=${[{
                        ...
                        ${value?.action === 'call-service' || value?.action === 'perform-action' ? html`
                            <ha-formfield .label="Use default entity">
                                <ha-switch
                                    ...
                                    .configValue="${(array ? array + "." : "") + (parseInt(index) == index ? index + "." : "") + configValueType + ".default_entity"}" 
  ...
  makeStyleEditor() {
    const panelKey = 'style_editor_panel'; // 此面板的唯一标识
