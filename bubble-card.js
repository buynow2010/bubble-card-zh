    ...
    <div class="module-store">
      ...
      ${e._rateLimitWarning?o.qy`
        <div class="bubble-info warning">
          ...
          <div class="content">
            <p>GitHub API 请求频率已达上限。模块列表从缓存加载。${e._rateLimitResetTime?`请在 ${function(e){const t=e-Date.now();if(t<=0)return"now";const n=Math.ceil(t/6e4);if(n<60)return`${n} minute${n>1?"s":""}`;const o=Math.floor(n/60),i=n%60;return 0===i?`${o} hour${o>1?"s":""}`:`${o} hour${o>1?"s":""} and ${i} minute${i>1?"s":""}`}(e._rateLimitResetTime)} 后重试。`:"请稍后重试。"}</p>
          </div>
        </div>
      `:""}
